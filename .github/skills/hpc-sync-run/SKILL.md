---
name: hpc-sync-run
description: 'Sync STARFORGE/SELECTED, scripts, and 3D-PDR-dev to an HPC host, preserve folder structure under a target remote path, and submit the HPC run script.'
...
## When To Use
- You want to push local STARFORGE/SELECTED, `scripts`, and `3D-PDR-dev` to an HPC environment.
...
- Source folders to sync:
  ...
  - `3D-PDR-dev`
- Snapshot run inputs:
   - Snapshot folder under SELECTED (example: `STARFORGE/SELECTED/<sim>/snapshots/810`)
...
3. Run the helper script:
   - `./scripts/sync_to_hpc.sh <host> <remote-path> STARFORGE/SELECTED scripts 3D-PDR-dev`
...
4. Organize snapshot-local run directory on HPC.
   - Build run folder inside the snapshot folder:
     - `<remote-path>/STARFORGE/SELECTED/<sim>/snapshots/<snapshot>/<run_dir>/`
...
- `STARFORGE/SELECTED`, `scripts`, and `3D-PDR-dev` are synced to the remote path.
